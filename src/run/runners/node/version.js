...
const getFullVersion = async function (version) {
  try {
    ...
    throw new UserError(
      `In the configuration property 'run_node_version': ${error.message}`,
    )
  }
}
...
const validateVersion = function ({ versionRange, version }, allowedVersions) {
  if (!satisfies(version, allowedVersions)) {
    throw new UserError(
      `In the configuration property 'run_node_version': version ${versionRange} must be ${allowedVersions}`,
    )
  }
}
